<template>
  <div class="graph-paper-container">
    ...
    >
      ...
      <!-- Buttons -->
      <div
        class="flex flex-col sm:flex-row gap-4 sm:space-x-4 mt-2 w-full sm:w-auto"
      >
        <button
          class="w-full sm:w-auto px-4 py-2 text-sm md:text-base font-semibold text-white bg-[#6c63ff] rounded-md hover:-translate-y-0.5 transition-transform duration-300 hover:bg-[#6c63ff]/80"
        >
          Let's get started
        </button>
        <button
          class="w-full sm:w-auto px-4 py-2 text-sm md:text-base font-semibold text-[#6c63ff] border border-[#6c63ff] rounded-md hover:-translate-y-0.5 transition-transform duration-300 hover:bg-[#6c63ff]/10"
        >
          Learn more
        </button>
      </div>
  ...
  <div class="container mx-auto px-4 py-12 sm:py-18">
    ...
    <div class="container mx-auto px-4">
      <!-- Solutions Grid -->
      <div
        class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 sm:mb-24 relative"
      >
        <!-- Line Container -->
...
<script setup>
// filepath: /c:/Users/ITS-Jerald/Desktop/ITS Hub Website Files/its.hub.website/app/pages/blogs.vue
import { ref, onMounted, onUnmounted } from 'vue';

const canvas = ref(null);
let ctx;
let animationFrameId;
let mouseX = -100; // Initialize mouseX off-screen
let mouseY = -100; // Initialize mouseY off-screen
let targetX = -100; // Target position for smooth transition
let targetY = -100; // Target position for smooth transition
...
const initCanvas = () => {
  ...
  const drawGrid = () => {
    ...
    const gridSize = 80; // Match the grid size to 80px
    const radius = 120; // Decrease the radius of the circular area

    ctx.clearRect(0, 0, width, height);
    ctx.strokeStyle = '#a0a0a0'; // Slightly darker gray
    ...
    for (let x = 0; x <= width; x += gridSize) {
      ...
      for (let y = 0; y <= height; y += gridSize) {
        ...
        if (distance < radius) {
          ...
          const distortion = (radius - distance) / 5; // Adjust the depth effect
          const distortedX = x + distortion * Math.cos(angle);
          const distortedY = y + distortion * Math.sin(angle);
